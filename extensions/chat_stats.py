...
def main(update, database):
    ...
    if user_obj:
        cursor.execute(
            "INSERT INTO users_list VALUES(%s, %s, %s, %s) ON DUPLICATE KEY UPDATE "
            "first_name=%s, last_name=%s, user_name=%s", add_user(user_obj))
    if chat_obj:
        cursor.execute(
            "INSERT INTO chats_list VALUES(%s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE "
            "title=%s, username=%s, first_name=%s, last_name =%s",
            add_chat(update['message']['chat']))
    database.commit()
    cursor.close()
...
def add_message(message, database, cursor):
    database.query(
        "SELECT status FROM chat_opt_status WHERE chat_id={}".format(message[
            'chat']['id']))
    ...
    if result and result[0]['status']:
        ...
        word_length = len(message['text'].split(
        )) if 'text' in message else None
        message_type = None
        for key in types:
            if key in message:
                message_type = key
        ...
        if message_type:
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS `{}stats`(user_id BIGINT NOT NULL, time_sent DATETIME NOT NULL, "
            ...
            cursor.execute(
                "INSERT INTO `{}stats` VALUES(%s, FROM_UNIXTIME(%s), %s, %s, %s)".format(
                    message['chat']['id']), values)
...
def add_user(user):
    ...
    entry = (user_id, first_name, last_name, username, first_name, last_name,
             username)
    return entry
...
def add_chat(chat):
    ...
    entry = (chat_id, chat_type, title, username, first_name, last_name, title,
