...
class TelegramInlineAPI(object):
    ...
    def answer_inline_query(self, results, **kwargs):
        ...
        if not isinstance(results, list):
            results = [results]
        package = {'inline_query_id': self.inline_query['id'], 'results': json.dumps(results)}
    ...
    def inline_query_result_sticker(self, sticker_file_id, **kwargs):
        ...
        """
        package = {'type': "sticker", 'id': str(uuid.uuid4()), 'sticker_file_id': sticker_file_id}
        package.update(kwargs)
        self.add_inline_query(package['id'])
        return package
    ...
    def inline_query_result_location(self, title, latitude, longitude, **kwargs):
        """
    ...
    def inline_result_venue(self, title, latitude, longitude, address, **kwargs):
        """
    ...
    def inline_query_result_contact(self, phone_number, first_name, **kwargs):
        ...
        """
        package = {'type': "contact", 'id': str(uuid.uuid4()), 'phone_number': phone_number, 'first_name': first_name}
        package.update(kwargs)
        self.add_inline_query(package['id'])
        return package
...
def input_venue_message_content(title, latitude, longitude, address, foursquare_id=None):
    """
    ...
    """
    package = {'latitude': latitude, 'longitude': longitude, 'title': title, 'address': address}
    if foursquare_id:
