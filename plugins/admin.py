...
def update_plugin_status(plugin_name):
    tg.database.query(
        'SELECT plugin_status FROM `{}blacklist` WHERE plugin_name="{}"'.format(
            chat_id, plugin_name))
    ...
    if row[0]['plugin_status'] != 2:
        ...
        if not has_permission:
            return False
    tg.cursor.execute(
        'UPDATE `{}blacklist` SET plugin_status = NOT `plugin_status` WHERE '
...
def create_plugin_keyboard():
    keyboard = list()
    tg.database.query(
        "SELECT b.plugin_name, pretty_name, plugin_status FROM `plugins` p "
        "LEFT JOIN `{}blacklist` b ON p.plugin_name=b.plugin_name WHERE "
        "b.plugin_name != \"admin\" AND hidden=0 AND inline_only=0;".format(
            chat_id))
    query = tg.database.store_result()
    ...
    for plugin in rows:
        button = {'callback_data':
                  "%%toggle%%{}".format(plugin['plugin_name']),
                  'text': "{} - {}"}
        ...
        if plugin['plugin_status'] == 0:
            ...
            button['text'] = button['text'].format(admin,
                                                   plugin['pretty_name'])
...
parameters = {
    ...
    'long_description':
    "The admin plugin allows you to enable and disable plugins in your group. Simply use the "
